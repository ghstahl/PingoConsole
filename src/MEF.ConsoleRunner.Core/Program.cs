...
using System.Linq;
using Fclp;
using MEF.ConsoleRunner.Core.Resources;
...
namespace MEF.ConsoleRunner.Core
{
    ...
    public static class Program
    {
        ...
        public static void MefRunnerEntryPoint(IAssemblyAccumulator assemblyAccumulator, string[] args)
        {
            try
            {
                ...
                twoColumnWidths = twoColumnWidths.ToFlexWidthColumns(twoColumnFlexIds, Console.BufferWidth, 16);
            ...
            catch (ParseException pe)
            {
                switch (pe.Result)
                {
                    case ParseResult.BadCommandArgument:
                        ConsoleHelper.DoConsoleErrorColor(
                            () => Console.WriteLine(Resources.Common.Format_UnknownCommand, pe.Text));
                        break;
                    default:
                        throw;
                }
            ...
            {
                ConsoleHelper.DoConsoleErrorColor(
                    () => Console.WriteLine(Resources.Common.Format_UnknownException, e.Message));
